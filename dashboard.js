...
function update_status() {
    try {
        ...
        if (id != null) {
            record_id = id;
            document.getElementById("top_txt").textContent = record_id;
            document.getElementById("top_txt").style.color = "#000000"
            get_ecg(device);
        } else {
            //Display error here
            Swal.fire({
                title: 'You cannot view this Page directly',
                icon: 'error',
                showConfirmButton: false,
                timer: 1500
            }).then(function (result) {
                change_window("record.html");
            });
        }
    }
    catch (e) {
...
function updateData(chart) {
    ...
    Swal.fire({
        title: 'ECG Record Loaded',
        text: `ECG record of ${record_id} is beign displayed now`,
        icon: 'success',
        showConfirmButton: false,
        timer: 2000
    })
...
async function get_ecg(device) {
    ...
    const docRef = doc(ft, device, record_id);
    ...
    if (docSnap.exists()) {
        ...
        Swal.fire({
            ...
            text: 'Oops! It seems like record is deleted. Redirecting...',
            icon: 'error',
            showConfirmButton: false,
            timer: 5000
        }).then(function (result) {
...
function initApp() {
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            document.getElementById('btn_records').addEventListener('click', replace_window, false);
